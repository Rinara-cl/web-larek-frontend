import { IProductView, FormErrors, PaymentForm, ContactForm } from "../../types";
...
export class ModalData extends Model<IModalData> {
	catalog: IProductView[];
	shoppingCart: IProductView[] = [];
	...
	addToShoppingCart(item: IProductView) {
		this.shoppingCart.push(item);
		this.emitChanges('basket:change', item);
	}

	removeFromShoppingCart(item: IProductView) {
	...
	setUserDataField(field: keyof PaymentForm, value: string) {
	...
	setUserContactsField(field: keyof ContactForm, value: string) {
		this.order[field] = value;
